...
/**
 * Common base of the RBAC controller tests: builds MockMvc on the shared RbacMvcTestConfig context, resets the
 * mocked services before each test and offers request helpers (signed tokens, JSON bodies).
 * <p>
 ...
 */
// MockMvc drives the DispatcherServlet without a real HTTP server. springSecurity() adds the
// springSecurityFilterChain bean of the context in front of it, exactly like the DelegatingFilterProxy does
// in Tomcat: without it the JWT filter and @PreAuthorize would be bypassed and every route would look open.
@SpringJUnitWebConfig(RbacMvcTestConfig.class)
abstract class AbstractRbacMvcTest {
    ...
    protected static MockHttpServletRequestBuilder postWith(String url, String jwt) {
