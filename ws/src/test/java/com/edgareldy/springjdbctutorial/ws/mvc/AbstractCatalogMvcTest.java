...
/**
 * Common base of the catalog controller tests: builds MockMvc on the shared CatalogMvcTestConfig context, resets
 ...
 */
// MockMvc + springSecurity() runs the real filter chain in front of the DispatcherServlet (see
// AbstractRbacMvcTest): the JWT filter and @PreAuthorize are exercised, only the services are mocked.
@SpringJUnitWebConfig(CatalogMvcTestConfig.class)
abstract class AbstractCatalogMvcTest {
    ...
    @BeforeEach
    void setUpMockMvc() {
        reset(categoryService, productService, authService);
